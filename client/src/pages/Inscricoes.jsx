    ...
    <GenericLayout>
      ...
      <div className="inscricoes-container">
          <div className="inscricoes-header">
            <h2>Inscrições</h2>
            <div className="inscricoes-header-right">
              {isAuth ? (
                <button
                  className={`btn-atualizar ${cooldown ? 'disabled' : ''}`}
                  onClick={async () => { if (cooldown) return; setCooldown(true); await carregarInscricoes(); setTimeout(() => setCooldown(false), 3000); }}
                  disabled={cooldown}
                >
              ...
              {!isAuth && <div className="login-note">Faça login para gerenciar suas inscrições. <a href="/login">Entrar</a></div>}
            </div>
          </div>

        <div className="panels-grid" style={{ padding: 12 }}>

          <div className="disponiveis-panel custom-scroll">
            <h3 className="panel-title">Eventos disponíveis</h3>
            {loadingEvents ? (
        ...
        {loading ? (
          ...
          <div className="minhas-panel">
            <h3 style={{ color: '#3e2723', marginBottom: 8 }}>Minhas Inscrições</h3>
            <div className="inscricoes-lista">
